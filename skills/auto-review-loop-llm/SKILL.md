...
Persist state to `research/REVIEW_STATE.json` after each round:
...
1. **Check `research/REVIEW_STATE.json`** for recovery
...
**Write `research/REVIEW_STATE.json`** with current state.

### Termination

1. Set `research/REVIEW_STATE.json` status to "completed"
