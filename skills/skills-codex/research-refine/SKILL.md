...
Long-running refinement sessions may fail mid-way (API timeout, context compaction, or session interruption). To avoid losing completed work, persist state to `research/refine/REFINE_STATE.json` after each phase boundary:
...
2. **On resume**:
   - Read all existing `research/refine/round-*.md` files and `score-history.md`
   - Recover `agent_id` for reviewer continuity
   - Resume from the next phase based on the saved `phase`
3. **On fresh start**, ensure `research/refine/` exists and proceed to Phase 0.
...
**Checkpoint:** Write `research/refine/REFINE_STATE.json` with `{"phase": "anchor", "round": 0, "agent_id": null, "last_score": null, "last_verdict": null, "status": "in_progress", "timestamp": "<now>"}`.
...
**Checkpoint:** Update `research/refine/REFINE_STATE.json` with `{"phase": "review", "round": 1, "agent_id": "<saved>", "last_score": <parsed>, "last_verdict": "<parsed>", ...}`.
...
**Checkpoint:** Update `research/refine/REFINE_STATE.json` with `{"phase": "review", "round": N, "agent_id": "<saved>", "last_score": <parsed>, "last_verdict": "<parsed>", ...}`.
