...
1. **Locate packages**: Treat `PAPER_LIBRARY` as a package library, not a bare PDF dump. Prefer this order:
   ...
   ```
   SCRIPT=$(find skills/research-lit/scripts ~/.agents/skills/research-lit/scripts ~/.codex/skills/research-lit/scripts -name "scan_local_paper_packages.py" 2>/dev/null | head -1)
