...
```
/paper-slides "writing/paper/"     # → Beamer PDF + PPTX + 演讲稿 + Q&A 预案
/paper-poster "writing/paper/"     # → A0/A1 海报 PDF + 可编辑 PPTX + SVG
```
...
- **2026-03-26** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 📄 **文档输入** — 放一个 `research/RESEARCH_BRIEF.md` 到项目根目录，`/idea-discovery` 和 `/research-pipeline` 自动检测。复杂研究方向不用挤在一句话里。[模板](templates/RESEARCH_BRIEF_TEMPLATE.md)
...
> | `compact` | `false` | 生成精简摘要文件（`research/IDEA_CANDIDATES.md`、`research/findings.md`、`research/EXPERIMENT_LOG.md`），适合短 context 模型和 session 恢复 |
...
输出 `research/IDEA_REPORT.md`（排名后的 idea）+ `research/refine/FINAL_PROPOSAL.md`（精炼后的方案）+ `research/refine/EXPERIMENT_PLAN.md`（实验路线图）。失败的 idea 也记录在案，避免重复踩坑。
...
1. 📋 **解析**实验计划（`research/refine/EXPERIMENT_PLAN.md`）
...
> 💡 **一键调用：** `/experiment-bridge` 自动读取 `research/refine/EXPERIMENT_PLAN.md`。也可指定：`/experiment-bridge "my_plan.md"`。
...
- 💾 **上下文压缩恢复** — 每轮结束后持久化状态到 `research/REVIEW_STATE.json`。如果上下文窗口满了触发自动 compact，工作流会从状态文件恢复断点继续——无需人工干预
...
**输出：** 一个可投稿的 `writing/paper/` 目录，含 LaTeX 源码、干净的 `.bib`（仅含实际引用）、编译好的 PDF。
...
| 常量 | 默认值 | 说明 |
|------|--------|------|
| `PAPER_LIBRARY` | `papers/` | 本地论文目录，搜外部之前先扫这里的 PDF |
...
- [x] **本地论文库扫描** — `/research-lit` 在外部搜索前先扫描本地 `papers/` 目录，复用已读论文
...
- [ ] **文档输入全流程** — 支持传入详细文档（如 `research/RESEARCH_BRIEF.md`）作为 `/research-pipeline` 或 `/idea-discovery` 的输入，替代一句话 prompt。很多研究方向需要详细的上下文（前期结果、约束条件、领域知识），一句话说不清楚。文档会被解析提取问题定义、约束、已有结果和具体需求
